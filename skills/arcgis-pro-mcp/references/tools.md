...
- `arcgis_pro_window_status`, `arcgis_pro_detach_window`
- `arcgis_pro_list_projects`

## Live Window And Active View

These require the attached Pro host. Pass `aprx_path=CURRENT`; an absolute project path is intentionally rejected for active-view tools.

- Read: `arcgis_pro_active_view_info`
- Focus: `arcgis_pro_open_map_view`, `arcgis_pro_open_layout_view`, `arcgis_pro_close_views`
- Camera: `arcgis_pro_set_active_view_extent`, `arcgis_pro_zoom_active_view_to_layer`, `arcgis_pro_zoom_active_view_to_all_layers`
- Redraw: `arcgis_pro_refresh_layer`
...
Map and layout discovery returns stable URI fields in addition to names. Layer discovery returns `long_name` and `uri`. Targeting helpers accept these identifiers and reject ambiguous short names instead of selecting the first match.
...
- Selection: `arcgis_pro_select_layer_by_attribute`, `arcgis_pro_select_layer_by_location`, `arcgis_pro_clear_map_selection`, `arcgis_pro_layer_selection_count`, `arcgis_pro_layer_selection_fids`. Selection writes verify the ArcPy derived count against `Layer.getSelectionSet()` before returning `selection_verified=true`; count/FID readers report only the exact selection set, never total rows.
