...
For native `sa.WeightedOverlay` / `WeightedOverlay_sa`, pass `in_weighted_overlay_table` as a JSON object with `evaluation_scale=[min,max,step]` and `rasters=[{raster,influence,field,remap}]`. Remap contains integer input/output pairs (or explicit `NODATA`/`RESTRICTED` outputs); integer percentage influences must sum to 100. The server constructs native WOTable/RemapValue objects and checks every embedded input path. Encoded WOTable strings and range/text remaps are not supported. Before TabulateArea, inspect the actual zone/class fields; do not invent GROUP_ID or silently replace a missing requested field.

The opt-in local-data [ecological acceptance runner](https://github.com/652036/arcgis-mcp-service/blob/main/docs/ECOLOGICAL_QA.md) verifies a declared source/resistance/cost/path/area chain through real stdio MCP, including independent full-cell numeric checks and failure injection. Its sequential gates are scoped to that runner, not universal enforcement on legacy tools; its example weights and selected forest patches are test assumptions, not ecological recommendations.
