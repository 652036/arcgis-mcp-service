...
def run_dissolve(
    ...
    dissolve = getattr(arcpy.management, "Dissolve", None) or getattr(arcpy.analysis, "Dissolve", None)
    if dissolve is None:
        raise RuntimeError("当前 ArcPy 没有 Dissolve")
    if df:
        dissolve(inf, out, df)
    else:
        dissolve(inf, out)
...
def run_export_features(arcpy: Any, in_features: str, out_path: str) -> None:
    ...
    exporter = getattr(arcpy.conversion, "ExportFeatures", None) or getattr(
        arcpy.management, "ExportFeatures", None
    ) or getattr(arcpy.management, "CopyFeatures", None)
    if exporter is None:
        raise RuntimeError("当前 ArcPy 没有 ExportFeatures/CopyFeatures")
    exporter(inf, out)
...
def run_export_table(arcpy: Any, in_table: str, out_path: str) -> None:
    ...
    intable = validate_input_path_optional(in_table, "in_table")
    out = validate_gp_output_path(out_path, "out_path")
    exporter = getattr(arcpy.conversion, "ExportTable", None) or getattr(
        arcpy.management, "ExportTable", None
    ) or getattr(arcpy.conversion, "TableToTable", None)
    if exporter is None:
        raise RuntimeError("当前 ArcPy 没有 ExportTable")
    if exporter is getattr(arcpy.conversion, "TableToTable", None):
        folder, name = os.path.split(out)
        exporter(intable, folder, name)
    else:
        exporter(intable, out)
