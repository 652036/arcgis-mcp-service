...
def insert_features(
    ...
    allowed_shape = {"SHAPE@WKT", "SHAPE@JSON"}
    cursor_fields: list[str] = []
    for f in fnames:
        token = f.upper()
        if token.startswith("SHAPE@"):
            if token not in allowed_shape:
                raise RuntimeError("几何请使用 SHAPE@WKT 或 SHAPE@JSON 字符串，不支持 SHAPE@ 对象")
            ...
            for i, val in enumerate(row_data):
                ...
                if fname.upper().startswith("SHAPE@"):
                    if not isinstance(val, str) or not val.strip():
                        raise RuntimeError(f"{fname} 必须为非空 WKT/JSON 字符串")
                    parsed.append(val)
                else:
